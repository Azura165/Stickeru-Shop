...
export async function middleware(request: NextRequest) {
  ...
  if (isAdminRoute && !isLoginPage) {
    ...
    // Admin route & sudah auth → lanjut tanpa cek maintenance
  ...
  if (isMaintenanceActive && !isMaintenancePage) {
    // Block semua public route → halaman maintenance
    return NextResponse.redirect(new URL("/maintenance", request.url));
  }

  if (!isMaintenanceActive && isMaintenancePage) {
    // Kalau maintenance OFF tapi user buka /maintenance → redirect ke home
